        ...
        <div className="libraryMain">
          ...
          <div className="libraryContentGrid">
            <div className="libraryLeft">
              ...
              <div className={`promptGrid ${viewMode === "list" ? "listMode" : ""}`}>{display.map((p) => <PromptSummaryCard key={p.id} prompt={p} isFav={favorites.includes(p.id)} onFav={() => toggleFavorite(p.id)} copyText={copyText} onView={() => setSelected(p)} setPage={setPage} setOptimizerSeed={setOptimizerSeed} />)}</div>
...
function BenefitStrip() {
  const benefits = [
    ["clock", "10K+ Hours Saved", "Through AI-optimized prompt templates"],
    ["users", "Role-Based Prompts", "Built for Dadi Coach roles and departments"],
    ["box", "RTCF Structured", "Consistent, reusable, management-ready"],
    ["shield", "Secure & Private", "All local uploads stay in your browser unless backend is enabled"],
    ["thumb", "Easy to Use", "Search, filter, copy, upload, and export with ease"]
  ];
  return (
    <div className="benefitStrip">
      {benefits.map(([icon, title, body]) => (
        <div className="benefitItem" key={title}><span><Icon name={icon} /></span><div><strong>{title}</strong><p>{body}</p></div></div>
      ))}
    </div>
  );
}
